...
function initData() {
  ...
  cardUtil.getLatestCardIds(30).then(cardIds => {

    console.log("latestCardIds = ", cardIds);
    cardIds = stableUnique(cardIds)
    console.log("uniqued cardIds = ", cardIds);

    cardUtil.getNewCardId().then(newCardId => {
      ...
      cardUtil.pushHistory(swipeData.swipeCardIds[swipeData.initIndex])
...
function openInit() {
  if (cardUtil.dbOpened()) {
    initData()
    return
  } else {
    setTimeout(() => {
      // 1. wait for the onsuccess event of openning db
      // 2. wait for sync end
      openInit()
    }, 500)
  }
...
function onChange(index) {
  ...
  if (index < mostNewReadIndex) {
    ...
    cardUtil.pushHistory(swipeData.swipeCardIds[mostNewReadIndex]).then(() => {

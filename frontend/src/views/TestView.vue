<template>
    <div id="nullNote" v-if="(cardProp.cardId == cardUtil.NoCardId) && (cardProp.card == null)">No card!</div>
    <div v-else>
        ...
        <ol v-else>
            <li class="title">no data</li>
        </ol>
        
        <change-card-view v-if="card.type in [0, 1, 2]" id="change-entrance"></change-card-view>
...
// const cardProp = defineProps(['cardId', 'card'])
const cardProp = {
    cardId: 1,
    card: null
}
...
watch(cardProp, (newCardProp, oldCardProp) => {
    ...
    if (newCardProp.cardId == cardUtil.NoCardId) {
        ...
        cardUtil.getCard(newCardProp.cardId).then(resp => {
            let data = resp.data.data
            card.type = data.type
            card.content = data.content
        })

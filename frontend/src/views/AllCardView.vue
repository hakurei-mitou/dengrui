...
function setData(allCardIds) {
  ...
  cardUtil.getLatestReadInAllCardIds(1).then(resp => {
    let readInAllCardId = resp.data.data[0]
    ...
    cardUtil.pushReadHistory(data.cardIds[data.initIndex], cardUtil.historyType.READINALL)

  })

}

cardUtil.getAllCardIds().then(resp => {
  let allCardIds = resp.data.data
  // console.log("allCardIds = ", allCardIds);
  setData(allCardIds)
})
...
function onChange(index) {
  ...
  currentIndex = index
  cardUtil.pushReadHistory(data.cardIds[index], cardUtil.historyType.READINALL)
